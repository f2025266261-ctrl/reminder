...
import { AppData, Deadline, Course, Priority } from '@/lib/types';
import { loadAppData, saveAppData } from '@/lib/storage';
import { parsePastedSyllabus, generateBalancedStudyTasks, formatDate } from '@/lib/planner';
import { Plus, Search, Filter, FileText, CheckCircle2, RefreshCw, X, Sparkles } from 'lucide-react';
...
export default function DeadlinesPage() {
  ...
  const handleAddDeadline = (e: React.FormEvent) => {
    ...
    const newDeadline: Deadline = {
      id: `d-${Date.now()}`,
      courseId: courseId || data.courses[0]?.id || 'c1',
      title,
      dueDate,
    ...
    const updatedDeadlines = [...data.deadlines, newDeadline];
    const updatedTasks = generateBalancedStudyTasks(updatedDeadlines, data.tasks);
    const updatedData: AppData = { ...data, deadlines: updatedDeadlines, tasks: updatedTasks };
  ...
  const handleParseSyllabus = () => {
    ...
    const newDeadlines: Deadline[] = parsedList.map((item, idx) => ({
      ...
      courseId: item.courseId || data.courses[0]?.id || 'c1',
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400 mt-1">
              Manage your course assignments or paste course syllabi to auto-generate deadlines.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-purple-600/20 border border-purple-500/30 px-4 py-2.5 text-xs sm:text-sm font-semibold text-purple-300 hover:bg-purple-600/30 transition-colors"
            >
        ...
        <div className="space-y-3">
          {filteredDeadlines.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/30 p-12 text-center text-slate-400">
              ...
              <p className="font-semibold text-slate-300">No deadlines match your filter.</p>
              <p className="text-xs text-slate-500 mt-1">Add a new deadline or paste course syllabus deadlines.</p>
            </div>
          ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-white">Add New Deadline</h3>
            ...
            <form onSubmit={handleAddDeadline} className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Optional details, chapter readings, submission link..."
                  value={notes}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-400">
              Paste course deadlines or full syllabus text below. The smart parser will extract assignment names, course codes, due dates, and estimated hours automatically.
            </p>

            <textarea
              rows={6}
              placeholder={`Example paste format:\nCS101 Problem Set 3 due 2026-09-28 5 hours\nMATH201 Calculus Midterm Exam due Oct 12 8 hrs\nENG102 Research Paper Draft due 10/18/2026`}
              value={pastedText}
